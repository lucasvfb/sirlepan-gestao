...
export default function Dashboard(){
  ...
  useEffect(()=>{(async()=>{try{const ini=new Date();ini.setDate(1);const start=ini.toISOString().slice(0,10);const[u,r,e,p,s,t,o]=await Promise.all([
    ...
    supabase.from("erp_transfers").select("id,status").in("status",["draft","sent","pending"]),
    supabase.from("erp_orders").select("id,status,pickup_at").in("status",["pending","confirmed","in_production","ready"])
  ...
  const overdue=useMemo(()=>entries.filter(e=>e.status==="pending"&&e.due_date&&new Date(e.due_date+"T23:59:59")<new Date()).length,[entries]);
  return <AppShell>
    <div className="topbar"><div><h1>Dashboard</h1><div className="subtitle">Visão consolidada das duas unidades da Sirlepan.</div></div></div>
    {erro&&<div className="notice error">{erro}</div>}
    <section className="grid4">
      <div className="card"><div className="metric-label">Faturamento do mês</div><div className="metric-value">{brl(totalRevenue)}</div><div className="metric-note">Lançamento diário consolidado</div></div>
      <div className="card"><div className="metric-label">Despesas pagas</div><div className="metric-value">{brl(expenses)}</div><div className="metric-note">No histórico financeiro carregado</div></div>
      <div className="card"><div className="metric-label">Contas vencidas</div><div className="metric-value">{overdue}</div><div className="metric-note">Pendências financeiras</div></div>
      <div className="card"><div className="metric-label">Transferências pendentes</div><div className="metric-value">{transfers.length}</div><div className="metric-note">Matriz → Resgate</div></div>
    </section>
    <section className="grid4">
      <div className="card"><div className="metric-label">Produtos</div><div className="metric-value">{products}</div><div className="metric-note">Cadastro iniciado limpo</div></div>
